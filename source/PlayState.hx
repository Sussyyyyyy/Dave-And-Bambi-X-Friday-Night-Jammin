...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['bro do you even know how to play this game', 0.2], //From 0% to 19%
		['severe skill issues', 0.4], //From 20% to 39%
		['skill issues', 0.5], //From 40% to 49%
		['bruhhhhhhhh', 0.6], //From 50% to 59%
		['nahhhhhh', 0.69], //From 60% to 68%
		['HAHA FUNNI NUMBER PLEASE LAUGH!!!!!!!!!!!!!!!!!!!!!!!!!!!', 0.7], //69%
		['good job', 0.8], //From 70% to 79%
		['gr8 job', 0.9], //From 80% to 89%
		['SICK!!!!!', 1], //From 90% to 99%
		['god your scaring me', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		detailsPausedText = "break time - " + detailsText;
		#end
		...
		switch (curStage)
		{
			...
			case 'universe':
				...
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('universe'));
				bg.active = true;
			...
			case 'thefandomtunnel':
				curStage = 'thefandomtunnel';
	
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('thefandomtunnel'));
				bg.active = true;
			...
			case 'theoretical':
				...
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('youfuckedupmyboy'));
				bg.active = true;
			...
			case 'baldisfarm':
				...
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('baldi-farm'));
				bg.active = true;
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Score: ' + songScore + ' | Skill Issues: ' + songMisses + ' | Rating: ' + ratingName;
		} else {
			scoreTxt.text = 'Score: ' + songScore + ' | Skill Issues: ' + songMisses + ' | Rating: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
		}
	...
	function openChartEditor()
	{
		...
		#if desktop
		DiscordClient.changePresence("CHEATING!!!!!!!", null, null, true);
		#end
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("HAHA HE DIED - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
